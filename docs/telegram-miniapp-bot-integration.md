...
| `o<operatorBase62>` | 运营 Bot 来源段；由 bot 服务按运营 Bot `tg_bot_id` 生成 | `o1LY7VK` |
...
- v1 payload 中未知 segment 会被跳过，不会导致整段 payload 失效；这保证新增来源段不影响既有路由、群组和 referral。
